...
const Cluster41Content = () => {
        ...
        if (res.ok) {
          ...
          const demoWeekOverrides: Record<string, (typeof DUMMY_WEEK_EXTRA)[string]> = {
            윤재윤: {
              ...
              teamPart: { teamName: "마케팅", partName: "전략파트" },
              roleLabel: "일반",
            ...
            전민경: {
              ...
              teamPart: { teamName: "사업개발전략기획", partName: "제휴협력사업파트" },
              roleLabel: "팀장(사업개발전략)",
            ...
            곽예원: {
              ...
              teamPart: { teamName: "브랜드커뮤니케이션", partName: "디지털마케팅전략파" },
              roleLabel: "운영진(앰배서더)",
            ...
            김의환: {
              ...
              teamPart: { teamName: "글로벌크로스보더커머스", partName: "퍼포먼스마케팅최적화" },
              roleLabel: "심화(에이전트파트장)",
      ...
      ? (
          {
            윤재윤: (
              <>
                현재 클럽은, <strong>26년 봄 시즌, 1주차</strong>를 진행 중에 있습니다.
              </>
            ),
            전민경: (
              <>
                현재 클럽은, <strong>26년 가을 시즌, 16주차</strong>를 진행 중에 있습니다.
              </>
            ),
            곽예원: (
              <>
                현재 클럽은, <strong>26년 겨울 시즌, 99주차</strong>를 진행 중에 있습니다.
              </>
            ),
            김의환: (
              <>
                현재 클럽은, <strong>26년 여름 시즌, 전환 주차</strong>를 진행 중에 있습니다.
              </>
            ),
  ...
  const [dbWeeklyData, setDbWeeklyData] = useState<DBWeekData[]>(isDemoMode ? DUMMY_WEEKLY_LIST : []);
  ...
  const getFormattedTeamPart = (date: string, week: DBWeekData) => {
    ...
    if (roleInfo && isAdminRole(roleInfo.role) && teamPart.teamName === "운영진") {
      ...
      const managedTeam = activeTeamPart?.managed_team_id ? teams.find((t) => t.id === activeTeamPart.managed_team_id) : null;
  ...
  const getPointsForWeek = (weekId: string) => {
    ...
    const star = weekPoints.filter((p) => p.point_type === "star").reduce((sum, p) => sum + p.points, 0);

    const lightning = weekPoints.filter((p) => p.point_type === "lightning").reduce((sum, p) => sum + p.points, 0);

    const shield = weekPoints.filter((p) => p.point_type === "shield").reduce((sum, p) => sum + p.points, 0);
  ...
  const getCumulativeInjeolmi = (weekEndDate: string) => {
    ...
    // 해당 주차 종료일까지의 모든 포인트
    const relevantWeekIds = dbWeeklyData.filter((w) => w.endDate <= weekEndDate).map((w) => w.id);

    const relevantPoints = userPoints.filter((p) => relevantWeekIds.includes(p.week_id));

    const totalShield = relevantPoints.filter((p) => p.point_type === "shield").reduce((sum, p) => sum + p.points, 0);

    const totalLightning = relevantPoints.filter((p) => p.point_type === "lightning").reduce((sum, p) => sum + p.points, 0);
  ...
  const isEnhancementSuccess = (weekId: string, activityTypeId: string): boolean => {
    // 1. 활동 완료 여부 확인
    const isCompleted = userActivities.some((a) => a.week_id === weekId && a.activity_type_id === activityTypeId);
    if (!isCompleted) return false;

    // 2. 2차 정보 기입 여부 확인
    const detail = activityDetails.find((d) => d.week_id === weekId && d.activity_type_id === activityTypeId);
    const hasSecondaryInfo = detail && ((detail.sub_title && detail.sub_title.trim() !== "") || (detail.output_links && detail.output_links.some((link) => link?.url && link.url.trim() !== "")));
    if (hasSecondaryInfo) return true;

    // 3. 48시간 경과 여부 확인
    const activity = weeklyActivities.find((wa) => wa.week_id === weekId && wa.activity_type_id === activityTypeId);
  ...
  const getWeeklyInfoRate = (weekId: string) => {
    ...
    const infoCount = infoTypeIds.filter((activityTypeId) => isEnhancementSuccess(weekId, activityTypeId)).length;
  ...
  const getWeeklyCompetencyRate = (weekId: string) => {
    ...
    const competencyCount = competencyTypeIds.some((activityTypeId) => isEnhancementSuccess(weekId, activityTypeId)) ? 1 : 0;
    // 소수점 올림 처리
  ...
  const getWeeklyExperienceRate = (weekId: string) => {
    ...
    experienceActivities.forEach((wa) => {
      ...
      if (cumulativeApproved >= minWeek && cumulativeApproved <= maxWeek) {
        ...
        if (typeInfo.count_once_in_total) {
          // 이미 이전 주차에서 완료했는지 확인
          const previouslyCompleted = userActivities.some((a) => a.activity_type_id === wa.activity_type_id && a.week_id !== weekId);
    ...
    const experienceCount = experienceTypeIds.filter((activityTypeId) => isEnhancementSuccess(weekId, activityTypeId)).length;
  ...
  useEffect(() => {
    ...
    const fetchWeeklyData = async () => {
      try {
        ...
        let weeksQuery = supabase.from("weeks").select("id, week_number, start_date, end_date, is_club_break, holiday_name, seasons (id, year, name, term_number)").lt("end_date", today).order("start_date", { ascending: false });
        ...
        if (userId) {
          ...
          const [weeklyGrowthResult, userPointsResult, userActivitiesResult, activityTypesResult, careerRecordsResult, weeklyActivitiesResult, activityDetailsResult] = await Promise.all([
            // user_weekly_growth
            ...
            userWeekIds.length > 0 ? supabase.from("weekly_activities").select("week_id, activity_type_id, is_active, opened_at").in("week_id", userWeekIds) : Promise.resolve({ data: [], error: null }),
            // activity_details - 2차 정보 (강화 성공 판단용)
            userWeekIds.length > 0 ? supabase.from("activity_details").select("week_id, activity_type_id, sub_title, output_links").eq("user_id", userId).in("week_id", userWeekIds) : Promise.resolve({ data: [], error: null }),
          ]);
          ...
          if (activityTypesResult.data) {
            ...
            activityTypesResult.data.forEach((at: { id: string; cluster_id: string; eligible_min_approved_weeks: number | null; eligible_max_approved_weeks: number | null; count_once_in_total: boolean }) => {
  ...
  const resultOptions = ["전체 (all)", "성장 (성공)", "성장 (실패)", "휴식 (개인)", "휴식 (공식)"];
  ...
  const paginatedDbData = filteredDbData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, i) => <img key={i} src="/images/0/cluster4/icon/icon - star.png" alt="star" className={`star-icon ${i >= rating ? "empty" : ""}`} />);
  };
                  ...
                  <div className="collection-content">
                    ...
                    <p className="collection-text">
                      {demoCollectionMessage ? (
                        ...
                        <>
                          현재 클럽은, <strong>{currentSeasonInfo ? `${currentSeasonInfo.year}년 ${currentSeasonInfo.name} 시즌, ${currentSeasonInfo.currentWeek}주차` : "로딩 중..."}</strong>를 {currentSeasonInfo?.isClubBreak ? `휴식(${currentSeasonInfo.holidayName || "공식"})` : "진행"} 중에
                          있습니다.
                        </>
                      )}
                    </p>
                ...
                <div className="details-card">
                  ...
                  <div className="details-content">
                    <div className="detail-row">
                      ...
                      <span className="detail-value">{startWeekInfo && startWeekInfo.year ? (startWeekInfo.isBreak ? `${startWeekInfo.year}년, ${startWeekInfo.seasonName} 시즌, 전환 주차` : `${startWeekInfo.year}년, ${startWeekInfo.seasonName} 시즌, ${startWeekInfo.weekNumber}주차`) : "-"}</span>
                    </div>
                    <div className="detail-row">
                      <span className="detail-label">성장 가능 주차</span>
                      <span className="detail-value">
                        <span className="number">{growthPeriodStats?.availableWeeks ?? "-"}</span>
                        <span className="orange-highlight">({growthPeriodStats?.availableSeasons ?? "-"})</span> <span className="white-text">개 주차</span>
                      </span>
                    </div>
                    <div className="detail-row">
                      ...
                      <span className="detail-value">
                        ...
                        {growthPeriodStats?.approvedSeasons ? <span className="orange-highlight">({growthPeriodStats.approvedSeasons})</span> : null} <span className="white-text">개 주차</span>
                      </span>
                    </div>
                    <div className="detail-row">
                    ...
                    <div className="detail-row">
                      ...
                      <span className="detail-value">
                        <span className="number">{growthPeriodStats?.restWeeks ?? "-"}</span>
                        {growthPeriodStats?.restSeasons ? <span className="orange-highlight">({growthPeriodStats.restSeasons})</span> : null} <span className="white-text">개 주차</span>
                      </span>
                    </div>
                    <div className="detail-row">
            ...
            <div className="filter-sheet-overlay">
              <div className="filter-sheet" onMouseDown={(e) => e.stopPropagation()}>
                ...
                <div className="filter-sheet-body">
                  ...
                  <select className="filter-sheet-select" value={draftSeason} onChange={(e) => setDraftSeason(e.target.value)}>
                  ...
                  <select className="filter-sheet-select" value={draftResult} onChange={(e) => setDraftResult(e.target.value)}>
          ...
          <div className="weekly-cards">
            {isLoadingWeeks ? (
              ...
              (isMobile ? filteredDbData.slice(0, mobileVisibleCount) : paginatedDbData).map((week) => {
                ...
                if (isMobile) {
                  return (
                    <div key={week.id} className={`weekly-card weekly-card--mobile ${isExpanded ? "is-expanded" : ""}`}>
                      <Link href={weekHref} className="weekly-card-main" style={{ textDecoration: "none", color: "inherit" }}>
                        <div
                          className={`weekly-card-image ${week.growthStatus === "휴식(개인)" || week.growthStatus === "실패" ? "rest-personal-overlay" : ""}`}
                          style={
                            {
                              "--divider-color": week.growthStatus === "실패" ? "#ff6b6b" : week.growthStatus === "휴식(개인)" ? "#65e3ff" : week.growthStatus === "휴식(공식)" ? "#ffea48" : week.growthStatus === "진행 중" ? "#9b59b6" : week.growthStatus === "집계 중" ? "#ff1493" : "#9dfa07",
                            } as React.CSSProperties
                          }
                        >
                          <img
                            src={getWeekImagePath(week) as string}
                            alt={`${week.seasonYear}년, ${week.seasonName} 시즌, ${week.isBreakSeason ? "전환 주차" : `${week.weekNumber}주차`}`}
                            onError={(e) => {
                          ...
                          <div className="image-badges">
                            <div
                              className={`badge-tag ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""} ${week.growthStatus === "진행 중" ? "in-progress" : ""} ${week.growthStatus === "집계 중" ? "counting" : ""}`}
                            >
                              {week.growthStatus.includes("휴식") ? week.growthStatus : `성장(${week.growthStatus})`}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="weekly-card-content">
                          <div className="weekly-card-header">
                            <h4 className="weekly-card-title">
                              {week.seasonYear}년, <span className="season-name-fixed">{week.seasonName}</span> 시즌,{" "}
                              {week.isBreakSeason ? (
                                <>
                                  <span className="week-name-fixed">전환</span> 주차
                                </>
                              ) : (
                                <>
                                  <span className="week-name-fixed">{week.weekNumber}</span>주차
                                </>
                              )}
                            </h4>
                              ...
                              <>
                                <div className="weekly-card-details-top">
                                  <div className="detail-chip">
                                    <strong>[팀]</strong> {teamPart.teamName || "-"}
                                  </div>
                                  <div className="detail-chip">
                                    <strong>[파트]</strong> {teamPart.partName || "-"}
                                  </div>
                                  <div className="detail-chip">
                                    <strong>[역할]</strong> {roleInfo?.roleLabel || "-"}
                                  </div>
                                </div>
                                ...
                                <div className="weekly-card-details-grid">
                                  <div className="detail-row">
                                    ...
                                    <span className="v">
                                      {isRest ? "-" : `${infoRate.rate}%`}{" "}
                                      <span className="sub">
                                        ({isRest ? "-" : infoRate.count}/{infoRate.total})
                                      </span>
                                    </span>
                                  </div>
                                  <div className="detail-row">
                                    ...
                                    <span className="v">
                                      {isRest ? "-" : `${competencyRate.rate}%`}{" "}
                                      <span className="sub">
                                        ({isRest ? "-" : competencyRate.count}/{competencyRate.total})
                                      </span>
                                    </span>
                                  </div>
                                  <div className="detail-row">
                                    ...
                                    <span className="v">
                                      {isRest ? "-" : `${experienceRate.rate}%`}{" "}
                                      <span className="sub">
                                        ({isRest ? "-" : experienceRate.count}/{experienceRate.total})
                                      </span>
                                    </span>
                                  </div>
                                  <div className="detail-row">
                                    ...
                                    <span className="v">
                                      {isRest ? "-" : `${careerRate.rate}%`}{" "}
                                      <span className="sub">
                                        ({isRest ? "-" : careerRate.count}/{careerRate.total})
                                      </span>
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="weekly-card-details-bottom">
                                  <div className="metric">
                                    단감 <strong>{weekPoints.star}</strong>
                                  </div>
                                  <div className="metric">
                                    인절미 <strong>{injeolmi}</strong>
                                  </div>
                                  <div className="metric">
                                    어흥 <strong>{Math.abs(weekPoints.lightning)}</strong>
                                  </div>
                                  <div className="metric">
                                    주차 평판 <strong>{weeklyReputationCounts[week.id] || 0}</strong>
                ...
                return (
                  <Link href={weekHref} key={week.id} className="weekly-card" style={{ textDecoration: "none", color: "inherit" }}>
                    ...
                    >
                      ...
                      {week.growthStatus === "휴식(개인)" && (
                        <div className="rest-message">
                          <span className="rest-text-line">
                            충분히 <span className="rest-emoji">🥰</span>
                          </span>
                      ...
                      {week.growthStatus === "실패" && (
                        <div className="rest-message">
                          <span className="rest-text-line">
                            값진 실패는 <span className="rest-emoji">😎</span>
                          </span>
                            ...
                            <>
                              <div className="info-group">
                                <span className="info-item team">
                                  <strong>[팀]</strong>{" "}
                                  <span className="text-gray" style={{ display: "inline-block", width: "109px", overflow: "hidden", textOverflow: "clip", whiteSpace: "nowrap" }}>
                                    {(teamPart.teamName || "-").length > 6 ? (teamPart.teamName || "-").slice(0, 6) + ".." : teamPart.teamName || "-"}
                                  </span>
                                </span>
                                ...
                                <span className="info-item part">
                                  <strong>[파트]</strong>{" "}
                                  <span className="text-gray" style={{ display: "inline-block", width: "109px", overflow: "hidden", textOverflow: "clip", whiteSpace: "nowrap" }}>
                                    {(teamPart.partName || "-").length > 6 ? (teamPart.partName || "-").slice(0, 6) + ".." : teamPart.partName || "-"}
                                  </span>
                                </span>
                              </div>
                              {/* 그룹 2: 역할 */}
                          ...
                          return (
                            <div className="info-group items">
                              ...
                              <span className="info-item with-icon">
                                ...
                                <strong className="number-value num-3">{weekPoints.star}</strong>개
                              </span>
                              <span className="info-divider">·</span>
                              <span className="info-item with-icon">
                                인절미
                                <img src="/images/0/cluster4/icon/icon - 인절미.png" alt="인절미" className="item-icon" />
                                <strong className="number-value num-3">{injeolmi}</strong>개
                              </span>
                              <span className="info-divider">·</span>
                              <span className="info-item with-icon">
                                어흥
                                <img src="/images/0/cluster4/icon/icon - 어흥.png" alt="어흥" className="item-icon" />
                                <strong className="number-value num-3">{Math.abs(weekPoints.lightning)}</strong>개
                              </span>
                            </div>
                          );
                        })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            <div className="weekly-card-main-progress">
                              <span className="progress-label">
                                <span className="dot">·</span> 주차 성장률{" "}
                                <strong>
                                  <span className="num-3">{isRest ? "-" : growthRate.rate}</span>%
                                </strong>
                              </span>
                              <div className="progress-bar-wrapper">
                                <div className="progress-bar">
                              ...
                              <span className="total-count">
                                <img src="/images/0/cluster4/icon/icon - 0.png" alt="leaf" className="leaf-icon" />총 <span className="num-3">{growthRate.total}</span> 개 중{" "}
                                <strong>
                                  <span className="num-3">{isRest ? "-" : growthRate.count}</span>
                                </strong>{" "}
                                개
                              </span>
                            </div>
                            ...
                            <div className={`weekly-card-stats-wrapper ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""}`}>
                              <div className="weekly-card-stats">
                                <span className="stat">
                                  <span className="dot">·</span> 실무 <span className={`highlight ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""}`}>정보</span> 강화율{" "}
                                  <strong>
                                    <span className="num-3">{isRest ? "-" : infoRate.rate}</span>%
                                  </strong>{" "}
                                  <span className="gray">
                                    (<span className="num num-2">{isRest ? "-" : infoRate.count}</span>/<span className="num-2">{infoRate.total}</span>)
                                  </span>
                                </span>
                                <span className="stat">
                                  <span className="dot">·</span> 실무 <span className={`highlight ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""}`}>역량</span> 강화율{" "}
                                  <strong>
                                    <span className="num-3">{isRest ? "-" : competencyRate.rate}</span>%
                                  </strong>{" "}
                                  <span className="gray">
                                    (<span className="num num-2">{isRest ? "-" : competencyRate.count}</span>/<span className="num-2">{competencyRate.total}</span>)
                                  </span>
                                </span>
                                <span className="stat">
                                  <span className="dot">·</span> 실무 <span className={`highlight ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""}`}>경험</span> 강화율{" "}
                                  <strong>
                                    <span className="num-3">{isRest ? "-" : experienceRate.rate}</span>%
                                  </strong>{" "}
                                  <span className="gray">
                                    (<span className="num num-2">{isRest ? "-" : experienceRate.count}</span>/<span className="num-2">{experienceRate.total}</span>)
                                  </span>
                                </span>
                                <span className="stat">
                                  <span className="dot">·</span> 실무 <span className={`highlight ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""}`}>경력</span> 강화율{" "}
                                  <strong>
                                    <span className="num-3">{isRest ? "-" : careerRate.rate}</span>%
                                  </strong>{" "}
                                  <span className="gray">
                                    (<span className="num num-2">{isRest ? "-" : careerRate.count}</span>/<span className="num-2">{careerRate.total}</span>)
                                  </span>
                                </span>
                              </div>
                              <div className="weekly-card-extra-stats">
                                <span className="stat">
                                  <span className="dot">·</span> <span className="label">주차 평판</span> <span className="num num-1">{isRest ? "-" : weeklyReputationCounts[week.id] || 0}</span>
                                ...
                                <span className="stat">
                                  <span className="dot">·</span> <span className="label">연계 동료</span> <span className="num num-1">{isRest ? "-" : (weeklyColleagueCounts[week.id] ?? 0)}</span>
                                  <span className="white">
                                    /<span className="num-1">3</span>
                                  </span>
                                </span>
                    ...
                    {/* 우측 성장 상태 */}
                    <div
                      className={`weekly-card-status-badge ${week.growthStatus === "실패" ? "fail" : ""} ${week.growthStatus === "휴식(개인)" ? "rest-personal" : ""} ${week.growthStatus === "휴식(공식)" ? "rest-official" : ""} ${week.growthStatus === "진행 중" ? "in-progress" : ""} ${week.growthStatus === "집계 중" ? "counting" : ""}`}
                    >
                      <span className="status-text">{week.growthStatus.includes("휴식") ? week.growthStatus : `성장 (${week.growthStatus})`}</span>
                      <img
                        src={
                          week.growthStatus === "진행 중"
                            ? "/images/0/cluster4/icon/icon%20-%20성장%20%28진행%20중%29.png"
                            : week.growthStatus === "집계 중"
                              ? "/images/0/cluster4/icon/icon%20-%20성장%20%28집계%20중%29.png"
                              : `/images/0/cluster4/icon/icon%20-%20${week.growthStatus.includes("휴식") ? week.growthStatus.replace("(", "%28").replace(")", "%29") : `성장%28${week.growthStatus}%29`}.png`
                        }
                        alt={week.growthStatus}
          ...
          {isMobile ? (
            <div className="weekly-pagination weekly-pagination--mobile">
              ...
              {filteredDbData.length > mobileVisibleCount && (
                <button type="button" className="weekly-load-more-btn" onClick={() => setMobileVisibleCount((c) => c + 10)}>
                  더 보기
                </button>
              )}
            </div>
            ...
            <div className="weekly-pagination">
              {totalPages > 0 ? (
                Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                  <span key={num} className={`page-num ${currentPage === num ? "active" : ""} ${num === totalPages ? "last" : ""}`} onClick={() => setCurrentPage(num)}>
                    {num}
                  </span>
                ))
              ) : (
